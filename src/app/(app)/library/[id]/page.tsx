...
export function generateStaticParams() {
  return knowledgeItems.map((k) => ({ id: k.id }));
}
...
export default async function KnowledgeDetailPage({
  ...
  const related = knowledgeItems.filter(
    (k) => item.connections.includes(k.id) || k.id === item.suggestedNext
  );
  const next = knowledgeItems.find((k) => k.id === item.suggestedNext);

  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <Button variant="ghost" size="sm" className="gap-1.5 -ml-2" render={<Link href="/library" />}>
      ...
      <header className="space-y-4">
        ...
        {item.url !== "#" && (
          <Button variant="outline" size="sm" className="gap-1.5" render={<a href={item.url} target="_blank" rel="noreferrer" />}>
            Open original
        ...
        <section className="space-y-3">
          ...
          {item.quotes.map((q) => (
            ...
            >
              ...
              <p className="text-sm italic leading-relaxed text-foreground/90">&ldquo;{q}&rdquo;</p>
            </blockquote>
          ))}
        </section>
      )}
